...
|---|---|---|---|---|---|---|
| DEF-025 | **The simulator would send librarian credentials to any address it was pointed at.** It authenticates as staff and sends that bearer token with every request, so whatever `-url` or `SIMULATOR_URL` names receives it. A mistyped or hostile value hands over a working staff session. | Medium | Second audit | `internal/simulator/agent.go` | **Closed** | The target is validated: plain `http` only for a local address, `https` otherwise. Nine cases tested. |
| DEF-024 | **The external catalogue address was an SSRF vector, and its response was decoded unbounded.** `OPENLIBRARY_BASE_URL` was used as given, so a value pointing at `169.254.169.254` or an internal host would make the server fetch it. Redirects were followed anywhere. The reply's record count was trusted, so a hostile response could force a very large number of allocations. Configuration is not the same as trustworthy: a copied `.env`, a compromised deployment or a typo is enough. | Medium | Second audit | `internal/books/openlibrary.go` | **Closed** | Host allowlist checked at startup so a bad value stops the service rather than surfacing mid-catalogue; redirects off the allowed host refused; the record count capped at the requested limit. Nine cases tested including cloud-metadata, private-network and lookalike-domain addresses. |
| DEF-023 | **The simulator was not isolated from real data.** `is_synthetic` existed as a column and nothing ever wrote it, so every simulated borrower was indistinguishable from a real student. Worse, the simulator chose a loan to return from **all** open loans, so against a database holding real records it would close a real student's loan and shelve a book they were still holding. Every action it took was a valid library operation, so nothing would have raised an error. | **Critical** | Second audit | `internal/simulator/`, `circulation_repo.go`, `member_service.go` | **Closed** | Simulated members are flagged `is_synthetic` on creation; `GET /loans?synthetic=true` restricts the simulator to its own borrowers; and the agent now **refuses to run at all** against an instance holding real borrower records unless `-allow-real-data` is passed explicitly. |
| DEF-022 | **A limiter outage disabled the login rate limit entirely.** On a Redis error the code allowed the request. An attacker who could make Redis unavailable, or simply wait for it to be, switched brute-force protection off. | High | Second audit | `internal/service/auth_service.go`, `middleware.go` | **Closed** | Falls back to an in-process counter. Weaker than Redis, but degraded beats disabled for a control whose purpose is to survive an adversary. |
| DEF-021 | **An access token outlived a password change by up to 15 minutes.** Refresh tokens were invalidated but the JWT already in an attacker's hands stayed valid, precisely during the window when it matters most, because a member changes their password when they suspect a compromise. | High | Second audit | `internal/transport/http/middleware/middleware.go` | **Closed** | The middleware compares the token's `iat` against the account's `tokens_invalid_before`. Costs one primary-key lookup per authenticated request; that is the price of being able to end a session now rather than in a quarter of an hour. |
| DEF-020 | **The fix for DEF-015 did nothing.** The check compared `now()`: the moment the refresh executed, against `tokens_invalid_before`. After any password change that comparison is trivially true, so it rejected nothing. The password-change race remained open behind a check that looked like it had closed it. | **Critical** | Second audit | `internal/service/auth_service.go`, `token_repo.go` | **Closed** | The comparison moved into the statement that consumes the token, where it belongs: `refresh_tokens.created_at >= users.tokens_invalid_before`. It is the token's own issue time that matters, not the moment somebody presents it. |
...
| DEF-029 | **Push notifications were never connected.** `notify.FCM` was written, reviewed and unit-tested, and `NewFCM` was never called anywhere in the program. In production only the mail sender was registered, so `senders["push"]` did not exist and the worker's `if !ok { return }` left every push message pending forever, waiting for a sender that would never appear. Nothing had accumulated only because no device had ever registered a token, there being no front end yet to register one. A feature can pass every test it has and still not be plugged in. | **High** | Found while setting up FCM for the front end, by asking where `NewFCM` was called | `cmd/api/main.go`, `internal/notify/fcm_auth.go` | **Closed** | `FCMTokenSource` builds the OAuth2 token function from a service account key, and `main.go` registers the sender when one is supplied. The project id is read out of the key rather than a second variable, so the two cannot disagree. In production an absent key now logs a warning naming the variable to set, instead of the channel failing in silence. |
| DEF-028 | **The activity simulator sent real mail to addresses that can never receive it.** Every synthetic member is created with an `@simulated.invalid` address, and the outbox worker handed each queued notification straight to the mail provider without asking whether the recipient was reachable. Resend accepted each call and then recorded a delivery it could never complete. **185 undeliverable sends went out against 2 genuine ones**, all from the sending domain `appmd.dev`, which also carries real mail. The scheduled simulation runs three times a day, so the count was growing on a timer. Not a data defect: the loans, members and copies it created are all correct. The defect is that a test fixture was allowed to spend a production resource. | **High** | Noticed by eye in the Resend dashboard, a surface no test watches | `internal/queue/worker.go`, `internal/notify/deliverable.go` | **Closed** | `notify.IsDeliverable` refuses the reserved domains of RFC 2606 and RFC 6761, and the worker marks such a message superseded rather than failed, since nothing went wrong and there is nothing to retry. The check is on the address, not on the synthetic flag, so a mistyped address is caught by the same rule. |
| DEF-027 | **Double-escaped names in HTML email.** `emailHTML` escapes any part that is not markup, and the templates escaped their interpolated values first, so a member whose name contained a bracket would have read `&lt;script&gt;` literally in their inbox. Not a security hole -- the output was safe, just wrong -- but member names come from a librarian's CSV upload, so the escaping path is one that matters. | Low | Test written alongside the feature | `internal/notify/notify.go` | **Closed** | Escaped exactly once, with the contract for markup versus plain-text parts written where the function is defined. |
| DEF-026 | **The OpenAPI specification was not valid YAML**, and had been committed that way for days. An edit inserted a schema by matching the text of an indented line, so `str.replace` took the first match and spliced a whole schema into the middle of another one, orphaning the rest of its properties and leaving a duplicate definition. The contract tests compared spec and router as **text**, and both of their assertions were true of a file no parser would accept. Python's YAML also tolerates duplicate keys, so a hand-check passed; Go's rejects them. | **High** | Found while removing em-dashes, by a parse that had never been run | `internal/transport/http/docs/openapi.yaml` | **Closed** | Structure repaired, and `TestSpecIsValidYAML` now parses the document and requires every path to declare an operation. |
| DEF-011 | **A due-soon reminder could be sent for a book already returned.** `NotifyDueSoon` queued reminders carrying only a title and a due date, so the worker had nothing to re-check against at delivery time: a reminder written last night described a situation that may have changed by morning. | Medium | Designing the notification worker | `internal/service/circulation_service.go` | **Closed** | Reminders carry `loan_id`, and the worker re-reads the loan before sending. Verified live: a reminder for a returned book was recorded as superseded and never sent. |
...
## Note on DEF-020: a fix that looked like a fix

DEF-015 was reported closed, with a regression test, and it was not closed. The
check compared the wrong instant, so it rejected nothing at all.

Two things let that through, and both are worth more than the bug:

1. **The test used a fake that modelled the implementation, not the rule.** The
   fake token store ignored the stamp entirely, so it agreed with whatever the
   service did. A fake that mirrors the code under test does not merely
   under-verify, it actively asserts that the wrong behaviour is correct. When
   the real rule was moved into SQL, the test failed, which is the first time it
   said anything true.
2. **"Fixed" was claimed on the strength of a passing test rather than an
   observation.** The end-to-end check run at the time exercised a *revoked*
   token, which failed for a different reason and looked like success.

The correction also moved the check to where it cannot be raced: inside the
statement that consumes the token. A comparison performed in Go, against a value
read a moment earlier, reopens the window it was meant to close.
...
## Note on DEF-026: a broken specification that every test called healthy

Removing em-dashes from the documentation surfaced a defect that had been
committed days earlier: the OpenAPI specification was not valid YAML.

An earlier edit inserted the `ExternalBook` schema by matching the text
`    Availability:`. That string also occurs, with deeper indentation, inside
`Book.properties`, and Python's `str.replace` took the first match. The result
spliced a whole schema into the middle of another one, orphaned `Book`'s
remaining properties, and left a duplicate definition further down.

Nothing noticed, for three reasons worth separating:

1. **The contract tests compare text, not structure.** They confirmed that every
   route appeared in the file and that every documented path had a route. Both
   were true of a file no parser would accept. Text matching cannot tell a
   document from its wreckage.
2. **Python's YAML parser accepts duplicate keys**, silently keeping the last.
   The initial check used Python and passed. Go's `yaml.v3` rejects them, which
   is how the second half of the damage came to light.
3. **Swagger UI fetches the spec in the browser**, so a malformed document would
   have shown as an empty page to the frontend team rather than as a failure in
   this repository.

`TestSpecIsValidYAML` now parses the document and requires every path to declare
at least one operation. It is the check that should have existed the moment the
specification became a build artifact rather than a note.
